angularApp.controller('Controller', ['$scope','$http',  function($scope,$http) {
	...
	angular.element(document).ready(function(){
		...
		var result="";
		$.ajax({
			 url:"https://app-photoemw.run.aws-usw02-pr.ice.predix.io/static/data/cities.json",
			 type:"get",
			 async:false,
			 dataType:"json",
		...
		//console.log(sourceObject);
		/* $.ajax({
			 url:"static/data/states.json",
			 type:"get",
			 async:false,
			 dataType:"json",
			 success:function(response){
				 
				 $.each(response.names,function(key,val){
					 var stateKey=val.Name;
					 cities[stateKey]=[];
					 $.ajax({
						 url:"https://www.whizapi.com/api/v2/util/ui/in/indian-city-by-state?project-app-key=8peehk659qb4l6vd6r776ess&stateid="+val.ID,
						 async:false,
						 type:"get",
						 success:function(cityresponse){
							// console.log(JSON.parse(cityresponse).Data);
							 var cityObj=JSON.parse(cityresponse).Data;
							 $.each(cityObj,function(key,val){
								 cities[stateKey].push(val.city);
							 });
